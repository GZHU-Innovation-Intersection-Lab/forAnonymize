...
def parse_args():
    ...
    ap.add_argument("--trustroute_label", default="TrustRoute(best_cfg)")
    ap.add_argument("--trustroute_seeds", nargs="*", default=None)
...
def main():
    ...
    test_jsonl = Path(args.gt_jsonl).resolve() if args.gt_jsonl else (exp_root / "test90" / f"{dataset}_test90.jsonl")
    ...
    if trustroute_best_dir.exists():
        ...
        if args.trustroute_seeds:
            allowed = {int(x) for x in args.trustroute_seeds}
            best_csvs = [p for p in best_csvs if _infer_seed_from_name(p.name) in allowed]
        for p in best_csvs:
            rows.append(
                summarize_one_csv(
                    dataset,
